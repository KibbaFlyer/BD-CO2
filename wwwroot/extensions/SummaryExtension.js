...
const SUMMARY_PROPS = ["CO2e summary"];
class SummaryExtension extends BaseExtension {
    ...
    onToolbarCreated() {
        ...
        this._button = this.createToolbarButton('summary-button', 'https://img.icons8.com/small/32/brief.png', 'Show CO2e Summary');
    ...
    async update() {
        if (this._panel) {
            ...
            if (selectedIds.length > 0) { // If any nodes are selected, compute the aggregates for them
                ...
                const dbids = await this.findLeafNodes(NOP_VIEWER.model);
                this._panel.update(NOP_VIEWER.model, dbids, SUMMARY_PROPS);
            }
